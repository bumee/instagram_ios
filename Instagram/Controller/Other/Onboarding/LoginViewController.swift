...
class LoginViewController: UIViewController {
    ...
    private func configureHeaderView(){
        // subview의 개수가 무조건 1개이게 강요
        // guard : 뒤에 조건이 false인 경우 괄호 안에 돌아감.
    ...
    @objc private func didTapLoginButton(){
        ...
        AuthManager.shared.loginUser(username: username, email: email, password: password, completion: { success in
            DispatchQueue.main.async {
                // 클로저 안에 있기 때문에, self 구문이 필요하다.
                if success {
                    self.dismiss(animated: true);
                }
                else{
                    let alert = UIAlertController(title: "Login Error", message: "We are not allowed to log you in.", preferredStyle: .alert);
